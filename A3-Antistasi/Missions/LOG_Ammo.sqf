...
_tipoVeh = if (_lado == enemySide) then {vehNATOAmmoTruck} else {vehCSATAmmoTruck};
...
[[friendlySide,civilian],"LOG",[format ["We've spotted an Ammotruck in an %1. Go there and destroy or steal it before %2:%3.",_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"Steal or Destroy Ammotruck",_marcador],_pos,false,0,true,"rearm",true] call BIS_fnc_taskCreate;
_camionCreado = false;
misiones pushBack ["LOG","CREATED"]; publicVariable "misiones";

waitUntil {sleep 1;(dateToNumber date > _fechalimnum) or ((spawner getVariable _marcador != 2) and !(lados getVariable [_marcador,sideUnknown] == friendlySide))};
_bonus = if (_dificil) then {2} else {1};
if ((spawner getVariable _marcador != 2) and !(lados getVariable [_marcador,sideUnknown] == friendlySide)) then
	{
	//sleep 10;
	...
	_camionCreado = true;
	if (_lado == enemySide) then {[_camion] call A3A_fnc_NATOcrate} else {[_camion] call A3A_fnc_CSATcrate};
	...
	_tipoGrupo = if (_dificil) then {if (_lado == enemySide) then {NATOSquad} else {CSATSquad}} else {if (_lado == enemySide) then {gruposNATOSentry} else {gruposCSATSentry}};
	//_cfg = if (_lado == enemySide) then {cfgNATOInf} else {cfgCSATInf};
	...
	waitUntil {sleep 1; (not alive _camion) or (dateToNumber date > _fechalimnum) or ({(_x getVariable ["spawner",false]) and (side group _x == friendlySide)} count crew _camion > 0)};
	...
	if ((not alive _camion) or ({(_x getVariable ["spawner",false]) and (side group _x == friendlySide)} count crew _camion > 0)) then
		{
		if ({(_x getVariable ["spawner",false]) and (side group _x == friendlySide)} count crew _camion > 0) then
			{
...
if (_camionCreado) then
	...
	deleteGroup _grupo1;
	deleteMarker _mrk;
	waitUntil {sleep 1; !([300,1,_camion,friendlySide] call A3A_fnc_distanceUnits)};
