...
_nombreEny = if (_sideEny == friendlySide) then
				{
				nameBuenos
				}
			else
				{
				if (_sideEny == oppositionSide) then {nameMuyMalos} else {nameMalos};
				};
_lado = _this select 2;
if (_lado == friendlySide) then {_lado = [friendlySide,civilian]};
...
if (_lado isEqualType []) then {_lado = friendlySide};
