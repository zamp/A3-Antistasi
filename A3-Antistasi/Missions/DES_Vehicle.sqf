...
if (spawner getVariable _marcador == 0) then
	...
	_tipo = if (_lado == enemySide) then {NATOCrew} else {CSATCrew};
	...
	if (_dificil) then
		...
		{
		waitUntil {sleep 1;({leader _grupo knowsAbout _x > 1.4} count ([spawnDistanceDefault,0,leader _grupo,friendlySide] call A3A_fnc_distanceUnits) > 0) or (dateToNumber date > _fechalimnum) or (not alive _veh) or ({(_x getVariable ["spawner",false]) and (side group _x == friendlySide)} count crew _veh > 0)};

		if ({leader _grupo knowsAbout _x > 1.4} count ([spawnDistanceDefault,0,leader _grupo,friendlySide] call A3A_fnc_distanceUnits) > 0) then {_grupo addVehicle _veh;};
		};

	waitUntil {sleep 1;(dateToNumber date > _fechalimnum) or (not alive _veh) or ({(_x getVariable ["spawner",false]) and (side group _x == friendlySide)} count crew _veh > 0)};

	if ((not alive _veh) or ({(_x getVariable ["spawner",false]) and (side group _x == friendlySide)} count crew _veh > 0)) then
		{
		...
		if ({(_x getVariable ["spawner",false]) and (side group _x == friendlySide)} count crew _veh > 0) then
			{
			["TaskFailed", ["", format ["AA Stolen in %1",_nombreDest]]] remoteExec ["BIS_fnc_showNotification",_lado];
			};
		[0,300*_bonus] remoteExec ["A3A_fnc_resourcesFIA",2];
		if (_lado == oppositionSide) then {[0,3] remoteExec ["A3A_fnc_prestige",2]; [0,10*_bonus,_posicion] remoteExec ["A3A_fnc_citySupportChange",2]} else {[3,0] remoteExec ["A3A_fnc_prestige",2];[0,5*_bonus,_posicion] remoteExec ["A3A_fnc_citySupportChange",2]};
