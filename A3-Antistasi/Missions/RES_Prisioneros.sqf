...
[[friendlySide,civilian],"RES",[format ["A group of POWs is awaiting for execution in %1. We must rescue them before %2:%3. Bring them to HQ",_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"POW Rescue",_marcador],_posicion,false,0,true,"run",true] call BIS_fnc_taskCreate;
...
for "_i" from 0 to _cuenta do
	...
	[_unit,"prisionero"] remoteExec ["A3A_fnc_flagaction",[friendlySide,civilian],_unit];
...
if ({alive _x} count _POWs == 0) then
	...
	{if (_x distance getMarkerPos friendlyRespawn < 500) then {[_cuenta,_x] call A3A_fnc_playerScoreAdd}} forEach (allPlayers - (entities "HeadlessClient_F"));
