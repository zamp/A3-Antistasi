...
while {!([player] call A3A_fnc_isMember)} do
	{
	_playerMembers = playableUnits select {([_x] call A3A_fnc_isMember) and (side group _x == friendlySide)};
	if !(_playerMembers isEqualTo []) then
		{
		if (player distance2D (getMarkerPos friendlyRespawn) > memberDistance) then
			{
	...
	if (_cuenta != 61) then
		...
		sleep 1;
		if (_cuenta == 0) then {player setPos (getMarkerPos friendlyRespawn)};
		}
	else
		{
		sleep 60;
