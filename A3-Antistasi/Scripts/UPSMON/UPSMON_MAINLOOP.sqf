			...
			case "FIREMISSION":
			{
				If (count _attackpos > 0 || count (_grp getvariable ["UPSMON_Artifiremission",[]]) > 0) then
				{
					If (_grp getvariable ["UPSMON_OnBattery",false]) then
					{
						...
						_amigos = if (side _grp == enemySide) then {{((side (group _x) == civilian) or (side (group _x) == side (leader _grp))) and (_x distance _attackPos < 100)} count allUnits} else {{(side (group _x) == side (leader _grp)) and (_x distance _attackPos < 100)} count allUnits};
