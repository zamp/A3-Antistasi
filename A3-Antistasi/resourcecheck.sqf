...
while {true} do
	...
	_suppBoost = 1+ ({lados getVariable [_x,sideUnknown] == friendlySide} count puertos);
	_recAddSDK = 25;//0
	_hrAddBLUFOR = 0;//0
	_popFIA = 0;
	_popAAF = 0;
	_popCSAT = 0;
	_popTotal = 0;
	_bonusFIA = 1 + (0.25*({(lados getVariable [_x,sideUnknown] == friendlySide) and !(_x in destroyedCities)} count fabricas));
	{
	_ciudad = _x;
	_recAddCiudadSDK = 0;
	_hrAddCiudad = 0;
	...
	_multiplicandorec = if (_power != friendlySide) then {0.5} else {1};
	...
	if (_ciudad in destroyedCities) then
		...
		switch (_power) do
			{
			case friendlySide: {[-1,_suppBoost,_ciudad] spawn A3A_fnc_citySupportChange};
			case enemySide: {[1,-1,_ciudad] spawn A3A_fnc_citySupportChange};
			case oppositionSide: {[-1,-1,_ciudad] spawn A3A_fnc_citySupportChange};
			};
		if (lados getVariable [_ciudad,sideUnknown] == enemySide) then
			{
	...
	// revuelta civil!!
	if ((_prestigeNATO < _prestigeSDK) and (lados getVariable [_ciudad,sideUnknown] == enemySide)) then
		{
		["TaskSucceeded", ["", format ["%1 joined %2",[_ciudad, false] call A3A_fnc_fn_location,nameBuenos]]] remoteExec ["BIS_fnc_showNotification",friendlySide];
		lados setVariable [_ciudad,friendlySide,true];
		_nul = [5,0] remoteExec ["A3A_fnc_prestige",2];
		_mrkD = format ["Dum%1",_ciudad];
		_mrkD setMarkerColor friendlyColor;
	...
	if ((_prestigeNATO > _prestigeSDK) and (lados getVariable [_ciudad,sideUnknown] == friendlySide)) then
		{
		["TaskFailed", ["", format ["%1 joined %2",[_ciudad, false] call A3A_fnc_fn_location,nameMalos]]] remoteExec ["BIS_fnc_showNotification",friendlySide];
		lados setVariable [_ciudad,enemySide,true];
		_nul = [-5,0] remoteExec ["A3A_fnc_prestige",2];
		_mrkD = format ["Dum%1",_ciudad];
	...
	if ((_popFIA > _popAAF) and ({lados getVariable [_x,sideUnknown] == friendlySide} count aeropuertos == count aeropuertos)) then {["end1",true,true,true,true] remoteExec ["BIS_fnc_endMission",0]};
	/*
	{
	_fabrica = _x;
	if (lados getVariable [_fabrica,sideUnknown] == friendlySide) then
		{
	...
	*/
	{
	_recurso = _x;
	if (lados getVariable [_recurso,sideUnknown] == friendlySide) then
		{
	...
	[petros,"taxRep",_texto] remoteExec ["A3A_fnc_commsMP",[friendlySide,civilian]];
	...
	bombRuns = bombRuns + (({lados getVariable [_x,sideUnknown] == friendlySide} count aeropuertos) * 0.25);
	[petros,"taxRep",_texto] remoteExec ["A3A_fnc_commsMP",[friendlySide,civilian]];
	[] call A3A_fnc_economicsAI;
	if (isMultiplayer) then
		{
		[] spawn A3A_fnc_assigntheBoss;
		difficultyCoef = floor ((({side group _x == friendlySide} count playableUnits) - ({side group _x != friendlySide} count playableUnits)) / 5);
	...
	if ((count antenasMuertas > 0) and (not(["REP"] call BIS_fnc_taskExists))) then
		...
		if ((lados getVariable [_marcador,sideUnknown] == enemySide) and (spawner getVariable _marcador == 2)) exitWith
			{
		...
		{
		_chance = 5;
		if ((_x in recursos) and (lados getVariable [_x,sideUnknown] == oppositionSide)) then {_chace = 20};
