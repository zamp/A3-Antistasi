...
//friendlyRespawn setMarkerPos [0,0,0];
friendlyRespawn setMarkerAlpha 0;
...
if (count _garrison > 0) then
	...
	_coste = 0;
	_hr = 0;
	if ({(alive _x) and (!captive _x) and ((side _x == enemySide) or (side _x == oppositionSide)) and (_x distance _posicion < 500)} count allUnits > 0) then
		{
		...
		{
		if ((side group _x == friendlySide) and (not(_x getVariable ["spawner",false])) and (_x distance _posicion < _size) and (_x != petros)) then
			{
			if (!alive _x) then
				{
