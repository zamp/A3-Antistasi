...
_threat = _threat + 2 * ({(isOnRoad getMarkerPos _x) and (getMarkerPos _x distance _posicion < spawnDistanceDefault)} count puestosFIA);

{
if (getMarkerPos _x distance _posicion < spawnDistanceDefault) then
	{
	_analizado = _x;
