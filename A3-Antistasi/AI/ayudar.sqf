...
if (_medico != _unit) then
	...
	if ((isPlayer _unit) and !(isMultiplayer))  then
		{
		if (([_medico] call A3A_fnc_canFight) and (_medico distance _unit > 3) and (_medico == _unit getVariable ["ayudado",objNull]) and !(_unit getVariable ["llevado",false]) and (allUnits findIf {((side _x == enemySide) or (side _x == oppositionSide)) and (_x distance2D _unit < 50)} == -1)) then {_medico setPos position _unit};
		};
