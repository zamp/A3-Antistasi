...
_origen = getMarkerPos friendlyRespawn;
...
while {(_size > 0)} do
	...
	_grupo = [_origen, friendlySide, _formato,false,true] call A3A_fnc_spawnGroup;
...
waitUntil {sleep 1;((not(_marcador in smallCAmrk)) or (lados getVariable [_marcador,sideUnknown] == enemySide) or (lados getVariable [_marcador,sideUnknown] == oppositionSide))};
/*
{_vehiculo = _x;
waitUntil {sleep 1; {_x distance _vehiculo < spawnDistanceDefault} count (allPlayers - (entities "HeadlessClient_F")) == 0};
deleteVehicle _vehiculo;
} forEach _vehiculos;*/
{_soldado = _x;
waitUntil {sleep 1; {_x distance _soldado < spawnDistanceDefault} count (allPlayers - (entities "HeadlessClient_F")) == 0};
